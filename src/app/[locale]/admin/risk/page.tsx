...
export default async function AdminRiskPage() {
  const now = new Date();
  const activeOrFundedWhere = getNonFixtureChallengeWhere({
    status: { in: ["active", "funded"] },
  });

  // Fetch all active + funded challenges with user + tier + pending picks
  const challenges = await prisma.challenge.findMany({
    where: activeOrFundedWhere,
    include: {
  ...
  const withMetrics = challenges.map((c) => {
    ...
    const drawdownRef = c.startBalance;
    const drawdownPct =
      drawdownRef > 0
        ? (Math.max(0, drawdownRef - c.balance) / drawdownRef) * 100
        : 0;
    const drawdownUsedPct = (drawdownPct / DRAWDOWN_LIMIT_PCT) * 100;

    // Daily loss only applies to phase challenges, not funded.
    const dailyLoss =
      c.phase !== "funded" ? Math.max(0, c.dailyStartBalance - c.balance) : 0;
    const dailyLossLimit =
      c.phase !== "funded"
        ? Math.floor((c.dailyStartBalance * DAILY_LOSS_LIMIT_PCT) / 100)
        : 0;
    ...
    <div className="space-y-10 max-w-6xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Live risk dashboard · drawdown limit {DRAWDOWN_LIMIT_PCT}% · daily
          loss limit {DAILY_LOSS_LIMIT_PCT}%
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          ...
          },
          {
            ...
            sub: "daily loss hit",
            color:
              paused.length > 0 ? "text-amber-400" : "text-muted-foreground",
          },
          {
            label: "Stuck Picks",
            ...
            sub: "pending >24h",
            color:
              stuckPicks.length > 0 ? "text-red-400" : "text-muted-foreground",
          },
          {
      ...
      <section>
        ...
        {nearDrawdown.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            All traders are within safe drawdown limits.
          </p>
        ) : (
            ...
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody>
                {nearDrawdown.map((c) => (
                  <tr
                    key={c.id}
                    className={`border-b border-border last:border-0 ${c.drawdownUsedPct >= 90 ? "bg-red-500/5" : ""}`}
                  >
                    <td className="px-4 py-3">
                      <Link
                        href={`/admin/users/${c.user.id}` as never}
                    ...
                    <td
                      className={`px-4 py-3 tabular-nums text-xs font-semibold ${riskColor(c.drawdownUsedPct)}`}
                    >
                    ...
                    <td
                      className={`px-4 py-3 tabular-nums text-xs font-bold ${riskColor(c.drawdownUsedPct)}`}
                    >
      ...
      <section>
        ...
        {nearDailyLimit.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No traders near daily loss limit today.
          </p>
        ) : (
          <div className="rounded-xl border border-border overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody>
                {nearDailyLimit.map((c) => (
                  <tr
                    key={c.id}
                    className={`border-b border-border last:border-0 ${c.dailyLossUsedPct >= 90 ? "bg-red-500/5" : ""}`}
                  >
                    <td className="px-4 py-3">
                      <Link
                        href={`/admin/users/${c.user.id}` as never}
                    ...
                    <td className="px-4 py-3 tabular-nums text-xs">
                      {fmt(c.dailyStartBalance)}
                    </td>
                    <td className="px-4 py-3 tabular-nums text-xs">
                      {fmt(c.balance)}
                    </td>
                    <td
                      className={`px-4 py-3 tabular-nums text-xs font-semibold ${riskColor(c.dailyLossUsedPct)}`}
                    >
                      -{fmt(c.dailyLoss)}
                    </td>
                    <td className="px-4 py-3 tabular-nums text-xs text-muted-foreground">
                      {fmt(c.dailyLossLimit)}
                    </td>
                    <td
                      className={`px-4 py-3 tabular-nums text-xs font-bold ${riskColor(c.dailyLossUsedPct)}`}
                    >
          ...
          <div className="rounded-xl border border-red-500/20 bg-red-500/5 overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              ...
              <tbody>
                {stuckPicks.map((p) => {
                  const hoursOld =
                    (now.getTime() - new Date(p.placedAt).getTime()) /
                    3_600_000;
                  return (
                    <tr
                      key={p.id}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-amber-500/20">
                  {["Trader", "Tier", "Phase", "Balance", "Unpauses At"].map(
                    (h) => (
                      <th
                        key={h}
                        className="text-left px-4 py-2.5 text-xs font-medium text-muted-foreground"
        ...
        {bigPositions.length === 0 ? (
          ...
          <div className="rounded-xl border border-border overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody>
                {bigPositions.map((c) => {
                  const balanceAtRiskPct =
                    c.balance > 0 ? (c.openStake / c.balance) * 100 : 0;
                  return (
                    <tr
                      key={c.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 tabular-nums text-xs text-pf-brand">
                        {fmt(c.openExposure)}
                      </td>
                      <td
                        className={`px-4 py-3 tabular-nums text-xs font-bold ${balanceAtRiskPct >= 10 ? "text-amber-400" : "text-muted-foreground"}`}
                      >
