...
const bodySchema = z.object({
  ...
  status: z.enum(["won", "lost", "void", "push"]),
});

export async function POST(req: NextRequest) {
  return withRouteMetric(
    {
      route: "POST /api/admin/picks/settle",
      source: "api:admin:picks:settle",
    },
    async () => {
      ...
      try {
        ...
        return NextResponse.json(
          {
            error: "pickId and status (won|lost|void|push) are required",
      ...
      const { pickId, status } = body as { pickId: string; status: SettleStatus };
