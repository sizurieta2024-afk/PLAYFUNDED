...
const parlayLegSchema = z.object({
  ...
  marketType: z.string().min(1).max(64),
  selection: z.string().min(1).max(128),
  odds: z.number().gte(1.01),
  linePoint: z.number().optional(),
});

const parlayBodySchema = z.object({
  isParlay: z.literal(true),
  challengeId: z.string().uuid(),
  stake: z.number().int().min(100).max(10_000_000),
  legs: z.array(parlayLegSchema).min(2).max(4),
});

const singlePickBodySchema = z.object({
  isParlay: z.literal(false).optional(),
  challengeId: z.string().uuid(),
  sport: z.string().min(1).max(64),
  league: z.string().min(1).max(64),
  event: z.string().min(1).max(128),
...
const postBodySchema = z.union([parlayBodySchema, singlePickBodySchema]);
...
export async function GET(req: NextRequest) {
  return withRouteMetric(
    ...
    async () => {
      ...
      if (!challengeId || !challengeIdSchema.safeParse(challengeId).success) {
        return NextResponse.json(
